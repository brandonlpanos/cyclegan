...
from dataset_Luke_tensornorm import AIAIRISDataset
...
cycleGAN_generated_dir = results_dir +  'Generated_images/'
make_dir(cycleGAN_generated_dir)
...
train_real_H = train_data_X.__getitem__(1287).unsqueeze(0)
train_real_Z = train_data_Y.__getitem__(576).unsqueeze(0)

val_real_H = test_data_X.__getitem__(187).unsqueeze(0)
val_real_Z = test_data_Y.__getitem__(34).unsqueeze(0)
...
def generate_result(real_image, gen_image, recon_image, epoch, val, result_dir, is_test=False, show=False):
    """
    ...
    """showim = gen_image.permute(1, 2, 0).cpu().numpy()
    plt.imshow(showim)
    plt.show()"""
    ...
    print("last epoch: ", val, config.epochs,epoch)
    
    genaiaim = gen_image[0].squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
    genirisim = gen_image[1].squeeze(0).permute(1, 2, 0).detach().cpu().numpy()

    ogaiaimage = real_image[0].squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
    ogirisimage = real_image[1].squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
    
    cycleaiaimage = recon_image[0].squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
    cycleirisimage = recon_image[1].squeeze(0).permute(1, 2, 0).detach().cpu().numpy()

    
    if epoch+1 == config.epochs:
        print("PRINT FINAL EPOCH")
        if val == 0:
            
            plt.title("AIA-IRIS generated image")
            plt.axis('off')
            plt.imshow(genaiaim)
            plt.savefig(cycleGAN_generated_dir + "gen_AIA_train_img.png")
            
            plt.title("IRIS-AIA generated image")
            plt.axis('off')
            plt.imshow(genirisim)
            plt.savefig(cycleGAN_generated_dir + "gen_IRIS_train_img.png")
            
            plt.title("Original AIA image")
            plt.axis('off')
            plt.imshow(ogaiaimage)
            plt.savefig(cycleGAN_generated_dir + "og_AIA_train_img.png")
            
            plt.title("Original IRIS image")
            plt.axis('off')
            plt.imshow(ogirisimage)
            plt.savefig(cycleGAN_generated_dir + "og_IRIS_train_img.png")
            
            plt.title("AIA reconstructed image")
            plt.axis('off')
            plt.imshow(cycleaiaimage)
            plt.savefig(cycleGAN_generated_dir + "recon_AIA_train_img.png")
            
            plt.title("IRIS reconstructed image")
            plt.axis('off')
            plt.imshow(cycleirisimage)
            plt.savefig(cycleGAN_generated_dir + "recon_IRIS_train_img.png")
            ...
            plt.axis('off')
            plt.imshow(genaiaim)
            plt.savefig(cycleGAN_generated_dir + "gen_AIA_val_img.png")
            
            plt.title("IRIS-AIA generated image")
            plt.axis('off')
            plt.imshow(genirisim)
            plt.savefig(cycleGAN_generated_dir + "gen_IRIS_val_img.png")
            
            plt.title("Original AIA image")
            plt.axis('off')
            plt.imshow(ogaiaimage)
            plt.savefig(cycleGAN_generated_dir + "og_AIA_val_img.png")
            
            plt.title("Original IRIS image")
            plt.axis('off')
            plt.imshow(ogirisimage)
            plt.savefig(cycleGAN_generated_dir + "og_IRIS_val_img.png")
            
            plt.title("AIA reconstructed image")
            plt.axis('off')
            plt.imshow(cycleaiaimage)
            plt.savefig(cycleGAN_generated_dir + "recon_AIA_val_img.png")
            
            plt.title("IRIS reconstructed image")
            plt.axis('off')
            plt.imshow(cycleirisimage)
            plt.savefig(cycleGAN_generated_dir + "recon_IRIS_val_img.png")
        
    
    titles = ['Real', 'Generated', 'Reconstructed']
    six_titles = ['AIA Real', 'AIA-IRIS Generated', 'AIA Reconstructed','IRIS Real', 'IRIS-AIA Generated', 'IRIS Reconstructed']
    iris_titles = ['IRIS Real', 'IRIS Generated', 'IRIS Reconstructed']
    if is_test:
    ...
    for i in range(len(images)):
        ...
        """
        ax.set_title(six_titles[title_i], color='red', fontsize = 16)
    """
...
for epoch in tqdm(range(epochs), desc="Epochs", disable=False):
    ...
    for real_X, real_Y in tqdm(zip(train_loader_X, train_loader_Y), desc="Train Batch", leave=False, disable=False):
        ...
        ***************************** Train Generator G_XtoY **************************
        ...
        """
        cycle_X_loss = l1_criterion(reconstructed_X, real_X) * config.cycleloss
        ...
        ***************************** Train Generator G_YtoX **************************
        ...
        8. Compute the cycle consistency loss by comparing the reconstructed reconstructed_Y images with real real_Y images of domain Y.
           Lambda for cycle loss is 10.0. Penalizing 10 times and forcing to learn the translation.
        """
        cycle_Y_loss = l1_criterion(reconstructed_Y, real_Y) * config.cycleloss
