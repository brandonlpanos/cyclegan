...
class AIAIRISDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.image_type == 'iris':
            ...
            clahe = cv.createCLAHE(clipLimit=2.0, tileGridSize=config.tilegridsize)
            out_im = clahe.apply(out_im)
            ...
            """plt.imshow(out_im, cmap='inferno')
            plt.show()"""
            ...
            showimg = augmented_im.permute(1, 2, 0).cpu().numpy()
...
class NormalizeMinMax(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        #print(torch.min(normalized_image), torch.max(normalized_image))
        
        
        
        """plt.imshow(normalized_image.permute(1, 2, 0).cpu().numpy())
        plt.show()"""
